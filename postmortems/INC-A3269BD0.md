# Post-Mortem: Cascading Failure: Triple failure: app-1 + redis + rabbitmq (3 services DOWN)

**Incident ID:** INC-A3269BD0
**Severity:** SEV3
...
A simulated incident was triggered by: Triple failure: app-1 + redis + rabbitmq. The incident affected 8 out of 6 infrastructure components (133% of the system). 3 component(s) went completely DOWN. 5 component(s) experienced degradation. The overall risk score for this scenario was 4.2/10.

## 2. Impact Assessment

**Components Affected:** 5 out of 6 (83% blast radius)

**Services DOWN:** RabbitMQ, Redis (cache), api-server-1
...
|-----|-------------|-------|----------|----------|--------|
| INC-A3269BD0-001 | Implement circuit breaker pattern on nginx (LB) -> api-server-1 dependency | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-002 | Enable failover configuration for api-server-1 | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-004 | Enable autoscaling for nginx (LB) | Platform Team | P1 | prevention | open |
| INC-A3269BD0-006 | Implement circuit breaker pattern on api-server-1 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-007 | Implement circuit breaker pattern on api-server-2 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-008 | Enable failover configuration for Redis (cache) | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-011 | Enable autoscaling for api-server-1 | Platform Team | P1 | prevention | open |
| INC-A3269BD0-013 | Implement circuit breaker pattern on nginx (LB) -> api-server-2 dependency | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-014 | Enable autoscaling for api-server-2 | Platform Team | P1 | prevention | open |
| INC-A3269BD0-016 | Implement circuit breaker pattern on api-server-1 -> RabbitMQ dependency | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-017 | Implement circuit breaker pattern on api-server-2 -> RabbitMQ dependency | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-018 | Enable failover configuration for RabbitMQ | SRE Team | P1 | mitigation | open |
| INC-A3269BD0-003 | Add health check monitoring for api-server-1 | SRE Team | P2 | detection | open |
| INC-A3269BD0-005 | Add health check monitoring for nginx (LB) | SRE Team | P2 | detection | open |
| INC-A3269BD0-009 | Add health check monitoring for Redis (cache) | SRE Team | P2 | detection | open |
| INC-A3269BD0-015 | Add health check monitoring for api-server-2 | SRE Team | P2 | detection | open |
| INC-A3269BD0-019 | Add health check monitoring for RabbitMQ | SRE Team | P2 | detection | open |
