# Post-Mortem: Cascading Failure: Pair failure: postgres + redis (2 services DOWN)

**Incident ID:** INC-A7F5AA97
**Severity:** SEV3
...
A simulated incident was triggered by: Pair failure: postgres + redis. The incident affected 6 out of 6 infrastructure components (100% of the system). 2 component(s) went completely DOWN. 4 component(s) experienced degradation. The overall risk score for this scenario was 4.0/10.
...
|-----|-------------|-------|----------|----------|--------|
| INC-A7F5AA97-001 | Implement circuit breaker pattern on api-server-1 -> PostgreSQL (primary) dependency | SRE Team | P1 | mitigation | open |
| INC-A7F5AA97-002 | Implement circuit breaker pattern on api-server-2 -> PostgreSQL (primary) dependency | SRE Team | P1 | mitigation | open |
| INC-A7F5AA97-003 | Enable failover configuration for PostgreSQL (primary) | SRE Team | P1 | mitigation | open |
| INC-A7F5AA97-005 | Enable autoscaling for api-server-1 | Platform Team | P1 | prevention | open |
| INC-A7F5AA97-007 | Enable autoscaling for api-server-2 | Platform Team | P1 | prevention | open |
| INC-A7F5AA97-009 | Implement circuit breaker pattern on api-server-1 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-A7F5AA97-010 | Implement circuit breaker pattern on api-server-2 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-A7F5AA97-011 | Enable failover configuration for Redis (cache) | SRE Team | P1 | mitigation | open |
| INC-A7F5AA97-004 | Add health check monitoring for PostgreSQL (primary) | SRE Team | P2 | detection | open |
| INC-A7F5AA97-006 | Add health check monitoring for api-server-1 | SRE Team | P2 | detection | open |
| INC-A7F5AA97-008 | Add health check monitoring for api-server-2 | SRE Team | P2 | detection | open |
| INC-A7F5AA97-012 | Add health check monitoring for Redis (cache) | SRE Team | P2 | detection | open |
