# Post-Mortem: Cascading Failure: Triple failure: nginx + redis + rabbitmq (3 services DOWN)

**Incident ID:** INC-2B460FC0
**Severity:** SEV3
...
A simulated incident was triggered by: Triple failure: nginx + redis + rabbitmq. The incident affected 7 out of 6 infrastructure components (117% of the system). 3 component(s) went completely DOWN. 4 component(s) experienced degradation. The overall risk score for this scenario was 4.6/10.

## 2. Impact Assessment

**Components Affected:** 5 out of 6 (83% blast radius)

**Services DOWN:** RabbitMQ, Redis (cache), nginx (LB)
...
|-----|-------------|-------|----------|----------|--------|
| INC-2B460FC0-001 | Enable failover configuration for nginx (LB) | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-003 | Implement circuit breaker pattern on api-server-1 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-004 | Implement circuit breaker pattern on api-server-2 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-005 | Enable failover configuration for Redis (cache) | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-007 | Implement circuit breaker pattern on nginx (LB) -> api-server-1 dependency | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-008 | Enable autoscaling for api-server-1 | Platform Team | P1 | prevention | open |
| INC-2B460FC0-010 | Implement circuit breaker pattern on nginx (LB) -> api-server-2 dependency | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-011 | Enable autoscaling for api-server-2 | Platform Team | P1 | prevention | open |
| INC-2B460FC0-013 | Implement circuit breaker pattern on api-server-1 -> RabbitMQ dependency | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-014 | Implement circuit breaker pattern on api-server-2 -> RabbitMQ dependency | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-015 | Enable failover configuration for RabbitMQ | SRE Team | P1 | mitigation | open |
| INC-2B460FC0-002 | Add health check monitoring for nginx (LB) | SRE Team | P2 | detection | open |
| INC-2B460FC0-006 | Add health check monitoring for Redis (cache) | SRE Team | P2 | detection | open |
| INC-2B460FC0-009 | Add health check monitoring for api-server-1 | SRE Team | P2 | detection | open |
| INC-2B460FC0-012 | Add health check monitoring for api-server-2 | SRE Team | P2 | detection | open |
| INC-2B460FC0-016 | Add health check monitoring for RabbitMQ | SRE Team | P2 | detection | open |
