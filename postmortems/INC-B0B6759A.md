# Post-Mortem: Cascading Failure: Triple failure: app-2 + redis + rabbitmq (3 services DOWN)

**Incident ID:** INC-B0B6759A
**Severity:** SEV3
...
A simulated incident was triggered by: Triple failure: app-2 + redis + rabbitmq. The incident affected 8 out of 6 infrastructure components (133% of the system). 3 component(s) went completely DOWN. 5 component(s) experienced degradation. The overall risk score for this scenario was 4.2/10.

## 2. Impact Assessment

**Components Affected:** 5 out of 6 (83% blast radius)

**Services DOWN:** RabbitMQ, Redis (cache), api-server-2
...
|-----|-------------|-------|----------|----------|--------|
| INC-B0B6759A-001 | Implement circuit breaker pattern on nginx (LB) -> api-server-2 dependency | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-002 | Enable failover configuration for api-server-2 | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-004 | Enable autoscaling for nginx (LB) | Platform Team | P1 | prevention | open |
| INC-B0B6759A-006 | Implement circuit breaker pattern on api-server-1 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-007 | Implement circuit breaker pattern on api-server-2 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-008 | Enable failover configuration for Redis (cache) | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-010 | Implement circuit breaker pattern on nginx (LB) -> api-server-1 dependency | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-011 | Enable autoscaling for api-server-1 | Platform Team | P1 | prevention | open |
| INC-B0B6759A-014 | Enable autoscaling for api-server-2 | Platform Team | P1 | prevention | open |
| INC-B0B6759A-016 | Implement circuit breaker pattern on api-server-1 -> RabbitMQ dependency | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-017 | Implement circuit breaker pattern on api-server-2 -> RabbitMQ dependency | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-018 | Enable failover configuration for RabbitMQ | SRE Team | P1 | mitigation | open |
| INC-B0B6759A-003 | Add health check monitoring for api-server-2 | SRE Team | P2 | detection | open |
| INC-B0B6759A-005 | Add health check monitoring for nginx (LB) | SRE Team | P2 | detection | open |
| INC-B0B6759A-009 | Add health check monitoring for Redis (cache) | SRE Team | P2 | detection | open |
| INC-B0B6759A-012 | Add health check monitoring for api-server-1 | SRE Team | P2 | detection | open |
| INC-B0B6759A-019 | Add health check monitoring for RabbitMQ | SRE Team | P2 | detection | open |
