# Post-Mortem: Cascading Failure: Triple failure: app-2 + postgres + redis (3 services DOWN)

**Incident ID:** INC-4C73BCE8
**Severity:** SEV3
...
A simulated incident was triggered by: Triple failure: app-2 + postgres + redis. The incident affected 8 out of 6 infrastructure components (133% of the system). 3 component(s) went completely DOWN. 5 component(s) experienced degradation. The overall risk score for this scenario was 4.2/10.

## 2. Impact Assessment

**Components Affected:** 5 out of 6 (83% blast radius)

**Services DOWN:** PostgreSQL (primary), Redis (cache), api-server-2
...
| Time | Event |
|------|-------|
| T+0s | api-server-2 failure detected: Component failure (simulated) |
...
|-----|-------------|-------|----------|----------|--------|
| INC-4C73BCE8-001 | Implement circuit breaker pattern on nginx (LB) -> api-server-2 dependency | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-002 | Enable failover configuration for api-server-2 | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-004 | Enable autoscaling for nginx (LB) | Platform Team | P1 | prevention | open |
| INC-4C73BCE8-006 | Implement circuit breaker pattern on api-server-1 -> PostgreSQL (primary) dependency | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-007 | Implement circuit breaker pattern on api-server-2 -> PostgreSQL (primary) dependency | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-008 | Enable failover configuration for PostgreSQL (primary) | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-010 | Implement circuit breaker pattern on nginx (LB) -> api-server-1 dependency | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-011 | Enable autoscaling for api-server-1 | Platform Team | P1 | prevention | open |
| INC-4C73BCE8-014 | Enable autoscaling for api-server-2 | Platform Team | P1 | prevention | open |
| INC-4C73BCE8-016 | Implement circuit breaker pattern on api-server-1 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-017 | Implement circuit breaker pattern on api-server-2 -> Redis (cache) dependency | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-018 | Enable failover configuration for Redis (cache) | SRE Team | P1 | mitigation | open |
| INC-4C73BCE8-003 | Add health check monitoring for api-server-2 | SRE Team | P2 | detection | open |
| INC-4C73BCE8-005 | Add health check monitoring for nginx (LB) | SRE Team | P2 | detection | open |
| INC-4C73BCE8-009 | Add health check monitoring for PostgreSQL (primary) | SRE Team | P2 | detection | open |
| INC-4C73BCE8-012 | Add health check monitoring for api-server-1 | SRE Team | P2 | detection | open |
| INC-4C73BCE8-019 | Add health check monitoring for Redis (cache) | SRE Team | P2 | detection | open |
