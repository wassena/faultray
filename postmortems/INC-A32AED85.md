# Post-Mortem: Cascading Failure: Triple failure: nginx + app-2 + postgres (3 services DOWN)

**Incident ID:** INC-A32AED85
**Severity:** SEV3
...
A simulated incident was triggered by: Triple failure: nginx + app-2 + postgres. The incident affected 6 out of 6 infrastructure components (100% of the system). 3 component(s) went completely DOWN. 3 component(s) experienced degradation. The overall risk score for this scenario was 5.0/10.

## 2. Impact Assessment

**Components Affected:** 4 out of 6 (67% blast radius)

**Services DOWN:** PostgreSQL (primary), api-server-2, nginx (LB)
...
|-----|-------------|-------|----------|----------|--------|
| INC-A32AED85-001 | Enable failover configuration for nginx (LB) | SRE Team | P1 | mitigation | open |
| INC-A32AED85-003 | Implement circuit breaker pattern on nginx (LB) -> api-server-2 dependency | SRE Team | P1 | mitigation | open |
| INC-A32AED85-004 | Enable failover configuration for api-server-2 | SRE Team | P1 | mitigation | open |
| INC-A32AED85-006 | Enable autoscaling for nginx (LB) | Platform Team | P1 | prevention | open |
| INC-A32AED85-008 | Implement circuit breaker pattern on api-server-1 -> PostgreSQL (primary) dependency | SRE Team | P1 | mitigation | open |
| INC-A32AED85-009 | Implement circuit breaker pattern on api-server-2 -> PostgreSQL (primary) dependency | SRE Team | P1 | mitigation | open |
| INC-A32AED85-010 | Enable failover configuration for PostgreSQL (primary) | SRE Team | P1 | mitigation | open |
| INC-A32AED85-012 | Implement circuit breaker pattern on nginx (LB) -> api-server-1 dependency | SRE Team | P1 | mitigation | open |
| INC-A32AED85-013 | Enable autoscaling for api-server-1 | Platform Team | P1 | prevention | open |
| INC-A32AED85-016 | Enable autoscaling for api-server-2 | Platform Team | P1 | prevention | open |
| INC-A32AED85-002 | Add health check monitoring for nginx (LB) | SRE Team | P2 | detection | open |
| INC-A32AED85-005 | Add health check monitoring for api-server-2 | SRE Team | P2 | detection | open |
| INC-A32AED85-011 | Add health check monitoring for PostgreSQL (primary) | SRE Team | P2 | detection | open |
| INC-A32AED85-014 | Add health check monitoring for api-server-1 | SRE Team | P2 | detection | open |
